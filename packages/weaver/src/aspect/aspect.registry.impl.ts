...
import { AdviceTarget, AnnotationContext, ReflectContext, _AnnotationLocationImpl } from '@aspectjs/reflect';
import { AdvicesFilter, AdvicesRegistry, AspectsRegistry } from './aspect.registry.type';
...
export class AspectsRegistryImpl implements AspectsRegistry {
    ...
    constructor(private _reflectContext: ReflectContext) {}
    ...
    register(...aspects: AspectType[]): void {
        (aspects ?? []).forEach((aspect) => {
            ...
            const annotationsContext = this._reflectContext.annotations;
    ...
    getAdvicesByTarget<T, A extends AdviceType>(
        ...
        (phases ?? []).forEach((phase) => {
            if (!targetRegistry[phase]) {
                // after, afterReturn & afterThrow advices precedence is higher first
                ...
                let advices = annotationContexts
                    ...
                    .sort((a1: Advice, a2: Advice) => {
                        ...
                        const a = this._reflectContext.annotations;
    ...
    private _getTarget<T>(obj: T): AdviceTarget<T> {
        return _AnnotationLocationImpl.unwrap(this._reflectContext.annotations.location.of(obj)).target;
    }
