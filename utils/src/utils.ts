 ...
 * @public
 */
export function _getReferenceConstructor(
  proto: object & { constructor: { new (...args: unknown[]): unknown } }
) {
  return (
    Reflect.getOwnMetadata(ASPECT_ORIGINAL_CTOR_KEY, proto) ?? proto.constructor
  );
}
 ...
 * @public
 */
export function setAspectOptions(
  target: Function,
...
export function assert(condition: boolean, msg?: string | (() => Error)) {
  if (__debug && !condition) {
    debugger;
    const e = isFunction(msg)
      ? (msg as Function)()
      : new Error(msg ?? 'assertion error');
